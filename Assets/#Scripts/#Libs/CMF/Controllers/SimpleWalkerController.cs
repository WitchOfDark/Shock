...
namespace CMF
{
    ...
    public class SimpleWalkerController : Controller
    {
        ...
        private Vector3 CalculateMovementDirection()
        {
            ...
            {
                _direction += tr.right * _input.x;
                _direction += tr.forward * _input.y;
            }
            else
            {
                //If a camera transform has been assigned, use the assigned transform's axes for movement direction;
                //Project movement direction so movement stays parallel to the ground;
                _direction += Vector3.ProjectOnPlane(cameraTransform.right, tr.up).normalized * _input.x;
                _direction += Vector3.ProjectOnPlane(cameraTransform.forward, tr.up).normalized * _input.y;
            }
        ...
        public override Vector3 GetSavedVelocity()
        {
            return lastVelocity;
        }

        //Return only the current movement velocity (without any vertical velocity);
        public override Vector3 GetSavedMovementVelocity()
        {
            return lastVelocity;
        }
